...
model = LIDM(latent_dim=z.shape[1], obser_dim=x.shape[1], sigma_x=.8,sigma_z=.3,alpha=.0, device=device).to(device)
...
trj_samples=np.random.randint(0, z_hat.shape[1],10)
for ii in trj_samples:
    ...
    plt.scatter(z_hat[:,ii,0],z_hat[:,ii,1], c='r', alpha=.5)
...
for ii in trj_samples:
    ...
    z_hat = z_hat.detach().cpu().numpy().squeeze()[1:, :, :]
    # z_hat = z_hat#2*(z_hat-z_hat.min(axis=0))/(z_hat.max(axis=0)-z_hat.min(axis=0))-1
    plt.scatter(z_hat[:,ii,0],z_hat[:,ii,1],c='b', alpha=.5)
    # plt.plot(z_hat[:, 0], z_hat[:, 1])
