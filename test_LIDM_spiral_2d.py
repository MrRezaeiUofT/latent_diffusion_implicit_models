...
model = LIDM(latent_dim=z.shape[1], obser_dim=x.shape[1], sigma_x=.2,sigma_z=.1,alpha=.0, device=device).to(device)
...
for ii in trj_samples:
    ...
    plt.scatter(z_hat[:,ii,0],z_hat[:,ii,1], c='r')
...
for ii in trj_samples:
    ...
    # z_hat = z_hat#2*(z_hat-z_hat.min(axis=0))/(z_hat.max(axis=0)-z_hat.min(axis=0))-1
    plt.scatter(z_hat[:,ii,0],z_hat[:,ii,1],c='b')
    # plt.plot(z_hat[:, 0], z_hat[:, 1])
