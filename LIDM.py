...
class LIDM(nn.Module):
    ...
    def forward(self, obsrv, obsr_enable):
        ...
        self.z_hat[0] = self.alpha * self.sigma_x*Variable(torch.randn(self.z_x_hat[0].shape)).to(self.device)
        ...
        for k in range(1, seq_len):
            ...
            if obsr_enable:
                temp = self.g_theta(output.clone()+eps_z/self.alpha) #+
                self.x_hat[k]=temp
                ...
                temp = self.g_theta(output+ eps_z/self.alpha)
                self.x_hat[k]=temp
