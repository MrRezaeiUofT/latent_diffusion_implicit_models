...
import LIDM
from LIDM import *
dataset = pickle.load(open("./NeuralData/example_data_m1_pp.p", "rb"))
batch_size=3
''' data'''
...
torch.save(model.state_dict(), 'final_model_MC_5mc_1000_V2.pt')

model.load_state_dict(torch.load('final_model_MC_5mc_1000_V2.pt'))
''''''
...
for ii in trj_samples:
    ...
    plt.savefig(save_result_path + 'Train-'+str(ii)+'-MC-with-obsr.png')
    plt.savefig(save_result_path + 'Train-'+str(ii)+'-MC-with-obsr.svg', format='svg')
    # plt.colse()
...
for ii in trj_samples:
    ...
    plt.title('with observations')
    plt.savefig(save_result_path + 'Val-'+str(ii)+'-MC-with-obsr.png')
    plt.savefig(save_result_path + 'Val-'+str(ii)+'-MC-with-obsr.svg', format='svg')
    # plt.close()
...
for ii in trj_samples:
    ...
    plt.title('with observations')
    plt.savefig(save_result_path + 'Test-'+str(ii)+'-MC-with-obsr.png')
    plt.savefig(save_result_path + 'Test-'+str(ii)+'-MC-with-obsr.svg', format='svg')
    # plt.close()
